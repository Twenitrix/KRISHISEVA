...
export default function ClaimsList() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-text-primary">{t('official.claims.title')}</h1>
        <p className="text-sm text-text-secondary">Verify and disburse pending insurance claims for your village</p>
      </div>
            ...
            <table className="min-w-full divide-y divide-border-default">
              <thead className="bg-surface-alt">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-text-secondary uppercase tracking-wider">Disaster / Crop</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-text-secondary uppercase tracking-wider">AI Score</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-text-secondary uppercase tracking-wider">Suggested Payout</th>
                ...
                {filteredClaims.map((claim) => (
                  <tr key={claim.id} className="hover:bg-surface-alt/50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-xs text-text-muted font-mono">{claim.id.split('-')[0].toUpperCase()}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-text-primary font-mono">
                      {claim.survey_number}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-text-primary">
                      <span className="capitalize">{claim.claimed_event_type}</span>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-text-primary">
                      {claim.overall_score !== undefined ? (
                        <span className={`px-2 py-0.5 rounded text-xs ${
                          claim.overall_score >= 80 ? 'text-status-verified bg-status-verified-bg font-semibold' :
                          claim.overall_score >= 50 ? 'text-status-pending bg-status-pending-bg' :
                          'text-status-rejected bg-status-rejected-bg font-medium'
                        }`}>
                          {claim.overall_score}%
                        </span>
                      ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-text-primary font-medium">
                      {claim.suggested_payout_amount ? (
                        ...
                        <span className="text-text-muted text-xs">AI review pending</span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end gap-2">
                        {claim.status === 'verified' ? (
                          ...
                          <Button variant="primary" size="sm" onClick={() => handleTriggerPayout(claim.id)}>
                            Trigger Payout
                          </Button>
                        ) : claim.status === 'payout_completed' ? (
                          <span className="text-xs text-status-verified font-medium flex items-center gap-1 py-1.5">
      ...
      >
        {loadingReport ? (
          ...
          <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
            ...
            {selectedClaim.status === 'verified' && (
              <form onSubmit={handleSubmitReview} className="border-t border-border-default pt-6 space-y-4 max-w-xl mx-auto">
                <h3 className="text-sm font-bold text-text-primary">Official Review & Final Decision</h3>
                ...
                {decision === 'approved' && (
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-text-primary">{t('official.review.approvedAmount')}</label>
                    <Input
                      type="number"
